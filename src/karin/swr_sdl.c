/*
** This file is from ../linux/RW_SDL.C
** Only implement SDL-software rendering. exclude SDL_GL
** OpenGL is supported by OpenGLES1.1, using EGL to creating OpenGL rendering context on X11 window. see gl_egl*.
**
...
int ax, ay, az, ax2, ay2, ax3, ay3;

static qboolean                 X11_active = false;

qboolean have_stencil = false;

SDL_Surface *surface; // set externlly for sharing to vkb surface.
...
int XLateKey(unsigned int keysym)
{
  ...
  case SDLK_COMMA:		key = K_ESCAPE; break;
    
  case SDLK_KP_ENTER:		key = K_ENTER; break;
...
void GetEvent(SDL_Event *event)
{
	...
	if (ax > 130) ax2 = -1;
	else if (ax < -130) ax2 = 1;
	else ax2 = 0;
	if (ay > -200) ay2 = 1;
	else if (ay < -460) ay2 = -1;
	else ay2 = 0;
	...
	if ( (ax2!=sp) && (ax2=-1) )
	{
	    keyq[keyq_head].key = K_LEFTARROW;
	...
	if ( (ax2!=sp) && (ax2=1) )
	{
	    keyq[keyq_head].key = K_RIGHTARROW;
	    keyq[keyq_head].down = true;
	    keyq_head = (keyq_head + 1) & 63;
		return;
    }
	...
	if ( (ay2!=fp) && (ay2=1) )
	{
		keyq[keyq_head].key = K_UPARROW;
	    keyq[keyq_head].down = true;
	    keyq_head = (keyq_head + 1) & 63;
		return;
	}
	else if ( (ay2!=fp) && (ay2=-1) )
	{
		keyq[keyq_head].key = K_DOWNARROW;
	    keyq[keyq_head].down = true;
	    keyq_head = (keyq_head + 1) & 63;
		return;
	}
	...
	unsigned int key;
#ifdef _HARMATTAN
	static Uint8 last_mouse_button = 0; // for mouse motion button.
#endif
	...
	case SDL_MOUSEBUTTONUP:
		last_mouse_button = event->button.state ? event->button.button : 0;
		karinSDLVKBMouseEvent(event->button.button, event->button.state, event->button.x, vid.height - event->button.y, karinHandleVKBAction);
	  break;
#ifdef _HARMATTAN
	case SDL_MOUSEMOTION:
		karinSDLVKBMouseMotionEvent(last_mouse_button /*event->motion.which: motion event has not button property, but don't care on Harmattan. 'which' is a mouse device, not a mouse button.*/, event->motion.state, event->motion.x, vid.height - event->motion.y, event->motion.xrel, -event->motion.yrel, karinHandleVKBAction);
		break;
#endif
#ifdef Joystick
...
*/
	...
	flags = SDL_DOUBLEBUF|SDL_HWSURFACE|SDL_HWPALETTE|SDL_FULLSCREEN;
#else
...
/*		accel_read(&ax, &ay, &az);
		...
		if (ax > 130) ax2 = 1;
		else if (ax < -130) ax2 = 2;
		else ax2 = 0;
		if (ay > -200) ay2 = 1;
		else if (ay < -460) ay2 = 2;
		else ay2 = 0;

		if ( ax2 == 1 )
		{
		  in_state->Key_Event_fp(K_LEFTARROW, true);
		  keyq_tail = (keyq_tail + 1) & 63;
			px = 1;
		}
		if ( ax2 == 2 )
		{
		  in_state->Key_Event_fp(K_RIGHTARROW, true);
		  keyq_tail = (keyq_tail + 1) & 63;
			px = 1;
		}
		if ( ay2 == 1 )
		{
		  in_state->Key_Event_fp(K_UPARROW, true);
		  keyq_tail = (keyq_tail + 1) & 63;
			px = 1;
		}
		if ( ay2 == 2 )
		{
		  in_state->Key_Event_fp(K_DOWNARROW, true);
		  keyq_tail = (keyq_tail + 1) & 63;
			px = 1;
		}*/
