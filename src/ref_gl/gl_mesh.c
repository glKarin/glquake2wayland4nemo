...
void GL_DrawAliasFrameLerp (dmdl_t *paliashdr, float backlerp)
{
	...
	int		index_xyz;
	float	*lerp;
	GLfloat *vs; // vertex array
	GLfloat *cs; // color array
	GLfloat *ts; // texcoord array
	GLenum primary_type;
	int vertex_count;
...
//		if ( currententity->flags & ( RF_SHELL_RED | RF_SHELL_GREEN | RF_SHELL_BLUE ) )
		...
		while (1)
		{
			...
			{
				count = -count;
				primary_type = GL_TRIANGLE_FAN;
			}
			else
			{
				primary_type = GL_TRIANGLE_STRIP;
			}
			...
			if ( currententity->flags & ( RF_SHELL_RED | RF_SHELL_GREEN | RF_SHELL_BLUE | RF_SHELL_DOUBLE | RF_SHELL_HALF_DAM) )
			{
				vs = calloc(count * 3, sizeof(GLfloat));
				...
				do
				{
					...
					vs[vertex_count * 3] = s_lerped[index_xyz][0];
					vs[vertex_count * 3 + 1] = s_lerped[index_xyz][1];
					vs[vertex_count * 3 + 2] = s_lerped[index_xyz][2];
				...
				qglDrawArrays(primary_type, 0, vertex_count);
				qglDisableClientState(GL_VERTEX_ARRAY);
				free(vs);
			}
			else
			{
				ts = calloc(count * 2, sizeof(GLfloat));
				vs = calloc(count * 3, sizeof(GLfloat));
				cs = calloc(count * 4, sizeof(GLfloat));
				do
				{
					// texture coordinates come from the draw list
					// karin(glArrayElement, true);
					ts[vertex_count * 2] = ((float *)order)[0];
					ts[vertex_count * 2 + 1] = ((float *)order)[1];
					index_xyz = order[2];
...
//					qglColor4f (l* shadelight[0], l*shadelight[1], l*shadelight[2], alpha);
					...
					vs[vertex_count * 3 + 1] = s_lerped[index_xyz][1];
					vs[vertex_count * 3 + 2] = s_lerped[index_xyz][2];
					cs[vertex_count * 4] = colorArray[index_xyz * 3];
					cs[vertex_count * 4 + 1] = colorArray[index_xyz * 3 + 1];
					cs[vertex_count * 4 + 2] = colorArray[index_xyz * 3 + 2];
					cs[vertex_count * 4 + 3] = 1.0;
	...
	else
	{
		while (1)
		{
			...
			if ( currententity->flags & ( RF_SHELL_RED | RF_SHELL_GREEN | RF_SHELL_BLUE ) )
			{
				cs = calloc(4 * count, sizeof(GLfloat));
				vs = calloc(3 * count, sizeof(GLfloat));
				do
				{
					index_xyz = order[2];
					order += 3;

					cs[vertex_count * 4] = shadelight[0];
					cs[vertex_count * 4 + 1] = shadelight[1];
					cs[vertex_count * 4 + 2] = shadelight[2];
					cs[vertex_count * 4 + 3] = alpha;
					vs[vertex_count * 3] = s_lerped[index_xyz][0];
					vs[vertex_count * 3 + 1] = s_lerped[index_xyz][1];
					vs[vertex_count * 3 + 2] = s_lerped[index_xyz][2];
			...
			else
			{
				...
				do
				{
					...
					cs[vertex_count * 4] = l * shadelight[0];
					cs[vertex_count * 4 + 1] = l * shadelight[1];
					cs[vertex_count * 4 + 2] = l * shadelight[2];
					cs[vertex_count * 4 + 3] = alpha;
					vs[vertex_count * 3] = s_lerped[index_xyz][0];
					vs[vertex_count * 3 + 1] = s_lerped[index_xyz][1];
					vs[vertex_count * 3 + 2] = s_lerped[index_xyz][2];
...
*/
			...
			point[2] = height;
//			height -= 0.001;
			vs[vertex_count * 3] = point[0];
			vs[vertex_count * 3 + 1] = point[1];
			vs[vertex_count * 3 + 2] = point[2];
