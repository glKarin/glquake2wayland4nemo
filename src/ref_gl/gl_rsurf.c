...
void DrawGLPoly (glpoly_t *p)
{
	...
	vs = calloc(p->numverts * 3, sizeof(GLfloat));
	ts = calloc(p->numverts * 2, sizeof(GLfloat));
	...
	for (i=0 ; i<MAX_LIGHTMAPS ; i++)
	{
		...
		for ( surf = gl_lms.lightmap_surfaces[i]; surf != 0; surf = surf->lightmapchain )
		{
			...
			for ( ; p ; p=p->chain)
			{
				...
				{
					GLfloat vs[] = {
						p->verts[0][0], p->verts[0][1], p->verts[0][2],
						p->verts[j-1][0], p->verts[j-1][1], p->verts[j-1][2],
						p->verts[j][0], p->verts[j][1], p->verts[j][2],
						p->verts[0][0], p->verts[0][1], p->verts[0][2],
					};
					// karinEnable(KARIN_VERTEX_ARRAY)
					qglVertexPointer(3, GL_FLOAT, 0, vs);
...
static void GL_RenderLightmappedPoly( msurface_t *surf )
{
	...
	GLfloat *ts0; // 多重纹理0
	GLfloat *ts1; // 多重纹理1
	GLint last_client_texture_state; // 初始的客户端纹理状态

	qglGetIntegerv(GL_CLIENT_ACTIVE_TEXTURE, &last_client_texture_state);
...
//PGM
		if (surf->texinfo->flags & SURF_FLOWING)
		{
			...
			{
				v = p->verts[0];
				vs = calloc(nv * 3, sizeof(GLfloat));
				ts0 = calloc(nv * 2, sizeof(GLfloat));
				ts1 = calloc(nv * 2, sizeof(GLfloat));
				for (i=0 ; i< nv; i++, v+= VERTEXSIZE)
				{
					// karin(glMultiTexCoord, false)
					ts0[i * 2] = v[3]+scroll;
					ts0[i * 2 + 1] = v[4];
					ts1[i * 2] = v[5];
					ts1[i * 2 + 1] = v[6];
				...
				qglTexCoordPointer(2, GL_FLOAT, 0, ts0);
				qglClientActiveTexture(GL_TEXTURE1);
				qglTexCoordPointer(2, GL_FLOAT, 0, ts1);
				qglVertexPointer(3, GL_FLOAT, 0, vs);
				qglDrawArrays(GL_TRIANGLE_FAN, 0, p->numverts);
				free(vs);
				free(ts0);
...
//==========
	...
	qglDisableClientState(GL_VERTEX_ARRAY);
	qglDisableClientState(GL_TEXTURE_COORD_ARRAY);
	qglClientActiveTexture(last_client_texture_state);
}
...
void R_DrawInlineBModel (void)
{
	...
	// draw the polygon
			...
			(!(psurf->flags & SURF_PLANEBACK) && (dot > BACKFACE_EPSILON)))
		{
			...
			else if ( qglMultiTexCoord4f && !( psurf->flags & SURF_DRAWTURB ) )
			{
...
// recurse down the children, front side first
	...
	for ( c = node->numsurfaces, surf = r_worldmodel->surfaces + node->firstsurface; c ; c--, surf++)
	{
		...
		{
			if ( qglMultiTexCoord4f && !( surf->flags & SURF_DRAWTURB ) )
			{
...
#else
		gl_lms.internal_format = GL_LUMINANCE_ALPHA; // kairn
#endif
	}
...
#else
		gl_lms.internal_format = GL_LUMINANCE; // karin
#endif
	}
	else
	{
